...
class ImportAzureCardsCsvTest(unittest.TestCase):
    def test_converts_ui_csv_to_source_export(self):
        ...
        with input_path.open("w", encoding="utf-8", newline="") as file:
            writer = csv.DictWriter(file, fieldnames=["ID", "Work Item Type", "Title", "Assigned To", "State", "Tags", "Created Date", "Target Date", "Closed Date", "Tempo gasto", "Created By", "Description", "Discussion", "Parent"])
            writer.writeheader()
            writer.writerow({"ID": "1", "Work Item Type": "Development", "Title": "Fix Java retry", "State": "Closed", "Tags": "backend;RabbitMQ", "Created Date": "2026-01-01T00:00:00Z", "Description": "Root cause analysis", "Discussion": "Chose smaller fix", "Parent": "99"})
