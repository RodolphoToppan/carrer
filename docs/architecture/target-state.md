...
6. **Artifact extraction**
   ...
   - current status: explicit local Markdown export of an accepted claim-based `ProfessionalArtifact` is isolated behind an in-memory `ClaimBasedArtifactExportCandidate` and human accept/reject review. Accept writes the exact persisted Markdown locally, persists an `ArtifactExportReceipt`, creates receipt traceability edges, and audits the action. Reject only audits. External export requires `artifact_safe`; internal export allows `internal` or `artifact_safe`. The `ProfessionalArtifact` remains accepted and is not marked exported. There is no upload, external publication, LinkedIn/job-board integration, or automatic pipeline export.
