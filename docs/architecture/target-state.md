...
7. **Interface adapters**
   ...
   - current status: a stdlib `argparse` CLI lives in `src/carrer/interfaces/` as a thin adapter over `CareerWorkflow`; it loads an explicitly provided existing JSON graph store, delegates query/status/integrity commands without persistence, and delegates narrow mutable human decision commands for current regenerated `ContributionCandidate` promotion/rejection and `ContributionAnalysis` acceptance/rejection. `analyses generate` is read-only and only returns the current deterministic in-memory analysis for a persisted `Contribution`. `analyses accept` and `analyses reject` require explicit actor and timestamp, regenerate the current analysis in the same process, delegate review to the application layer, verify the resulting state in memory, then save the graph through atomic replacement as the final critical operation. Analysis acceptance persists the accepted node and provenance; rejection records audit only. The CLI does not accept arbitrary analysis JSON or stdin payloads. CLI tests reload the saved JSON store to prove durable persistence.
