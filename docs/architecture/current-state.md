...
- `CareerClaim` can now be persisted through explicit candidate acceptance. The persisted claim preserves the accepted candidate statement without reformulation, keeps confidence and privacy unchanged, stores candidate identity and supporting refs as provenance, and is queryable. Rejection creates audit only. The pipeline and artifact generators do not consume persisted career claims yet.
