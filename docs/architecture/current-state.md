...
- `ContributionAnalysis` is a pure JSON-serializable in-memory contract only. It is read-only, uses only explicit `Contribution` fields and referenced `EvidenceNode` metadata, classifies impact indicators as signals rather than confirmed impact, requires explicit numeric metrics with units, performs no calculation or estimation, creates no audit record, and does not generate `CareerClaim`.
...
- contribution candidate discovery exists only as an explicit read-only query; `ContributionAnalysis` exists only as an explicit read-only in-memory query; automatic contribution creation, persisted Work-to-Impact analysis, impact scoring, and automatic `CareerClaim` generation are not implemented
...
- the legacy pipeline, inference, ingestion, and artifacts do not execute or import `ContributionAnalysis`
## Preservation Rules
