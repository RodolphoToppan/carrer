...
- `GraphIntegrityReport` is a global, JSON-serializable, in-memory report for the graph store itself. It remains structurally deterministic over observed graph structure and explicit canonical filters, contains no generation timestamp, links summary counts to the structural snapshot counts, and is separate from `ArtifactExportIntegrityReport`, which checks one local export receipt against files and export-specific edges. Invalid non-JSON values are represented internally by safe structural type descriptors, arbitrary `repr()` output is not copied into issues or report metadata, validator logic rejects reports that inject arbitrary strings into issue refs or paths, mismatch an issue code with the wrong severity, subject type, path, related refs, or metadata, use a path ref different from `subject_ref`, or use a subject prefix incompatible with the subject type; generation applies that same compatibility rule before exposing subjects. `snapshot.id` must be `graph_snapshot:<64 lowercase hex>`, and actor does not affect snapshot, fallback audit refs, issue IDs, or report IDs. Domain semantics are currently limited to persisted `Contribution` integrity; other domains remain outside this report's semantic scope.
