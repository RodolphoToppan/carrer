...
- `src/carrer/artifacts/` builds legacy Knowledge-based professional artifacts, renders Markdown, validates warnings, and preserves traceability. It also exposes an explicit read-only claim-based artifact API for caller-selected accepted `CareerClaim` IDs; that API validates claims and provenance edges, applies audience/privacy rules, preserves statements without rewriting, returns in-memory `resume_claims` or `linkedin_claims` contracts, and renders Markdown only when called directly.
...
- the legacy pipeline, inference, ingestion, and Knowledge-based artifacts do not execute or import `ContributionAnalysis` review, `CareerClaimCandidate` generation, or claim-based artifact generation
## Preservation Rules
