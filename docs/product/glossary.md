...
A graph integrity report is a deterministic, read-only, in-memory inspection of the graph store. Its base layer reports invalid node envelopes, edge envelopes and references, audit record envelopes, conservative persisted audit target references, duplicate edges, and non-canonical target refs. It also has limited semantic checks for explicitly persisted contracts where the invariant is stable; currently this includes conservative `Contribution`, `ContributionAnalysis`, `CareerClaim`, `ProfessionalArtifact`, `ArtifactExportReceipt`, and `ArtifactExportRepairReceipt` contract and provenance/export/governance checks. It includes a structural snapshot identity whose full canonical input is not exposed; the report exposes only the `graph_snapshot:<64 lowercase hex>` snapshot ID and counts, and summary counts must match those snapshot counts. Persisted filters are ordered and deduplicated. Dictionary key/node ID mismatch is detected even for non-textual node keys. Only canonical structural refs are exposed in issues; arbitrary strings found in keys, node IDs, edge endpoints, audit targets, or invalid IDs are replaced by deterministic opaque refs. Canonical refs with prefixes incompatible with the issue subject type are also converted to opaque fallbacks during generation. Invalid values are represented with safe structural descriptors, not arbitrary `repr()` output, actor is excluded from structural identity, and the validator rejects reports that insert arbitrary content into `subject_ref`, `related_refs`, `path`, issue metadata, pair an issue code with the wrong severity, subject type, path, related refs, or metadata, use a path ref different from `subject_ref`, or use a subject prefix incompatible with node, edge, or audit-record subjects. Generation and validation share that subject compatibility rule, so generated reports validate under the same contract. Issue metadata has a closed contract by issue code. It is global to the graph store and is distinct from export integrity, which validates one artifact export receipt against local files and export-specific traceability. Professional artifact inspection distinguishes legacy Knowledge-based artifacts from claim-based artifacts, checks persisted refs and real artifact-to-claim/knowledge/evidence edges. Artifact export receipt inspection checks only persisted receipt shape, deterministic identity, persisted refs/counts/hash/file-name structure, source artifact status/privacy compatibility, and real receipt-to-artifact/claim/evidence edges. Artifact export repair receipt inspection checks only persisted receipt shape, deterministic identity, persisted repair candidate/report/original receipt/audit refs, persisted repair actions, mutation counters, and safely correlated original repair acceptance audit data. It does not regenerate artifact drafts, export candidates, repair candidates, or export integrity reports; select claims; rewrite statements; render Markdown; read exported files; verify directories, final files, or temporary files; recalculate content hashes; apply repair; modify audits, original receipts, or repair receipts; persist reports; run artifacts; or execute the pipeline. Filesystem integrity, global repair, automatic reconciliation, automatic repair execution, automatic pipeline execution, and cryptographic protection against direct manual JSON rewriting remain outside this graph integrity layer.
